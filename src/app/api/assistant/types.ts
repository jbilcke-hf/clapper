...
export const llmGeneratedFacetZ = z.object({
  ...
  category: z
    ...
    .describe(
      `Category of the facet (can be one of: ${[
        'weather', // "rainy", "cloudy sky", "soft mist", etc..
        'clothes', // "roman toga", "brown gucci vest", etc..
        'era', // "early 1800", "in 1995", "during WWII", etc..
        'location', // "in the alps", "in a sand desert", etc..
  ...
  details: z
    .string()
    .describe(
      `Facet category type (can be one of: ${Object.keys(
        ClapSegmentCategory
      ).join(', ')})`
    ),
})

export const llmGeneratedTakesZ = z.array(
  z.object({
    summary: z.string().describe('the summary of the take'),
    composition: z
      .array(llmGeneratedFacetZ)
      .describe(
        `list of about 8-12 facets (some facets can be repeated eg. the character, sound etc but it wouldn't make sense to repeat some other type of facets such as the era, weather or camera. Use your best judgement!)`
      ),
  })
)
